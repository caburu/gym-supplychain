...
class SupplyChain2perStageEnv(SupplyChainEnv):
    ...
    def __init__(self, num_products=1,
                 initial_stocks=[0]*8, initial_supply=[[[60,60]]]*2, initial_shipments=[[[60,60]]]*2 + [[[20,20]]]*4,
        ...
        for i in range(2):
            nodes_info['Supplier'+str(i+1)] = {
                'initial_stock':initial_stocks[i], 'initial_supply':initial_supply[i],
                'stock_capacity':stock_capacities[i], 'stock_cost':stock_costs[i],
                'supply_capacity':supply_capacities[i], 'supply_cost':supply_costs[i],
                'destinations':['Factory1','Factory2'], 'dest_costs':[[dest_cost]*2]*num_products, 
                'ship_capacity':[ship_capacity]*2}
        for i in range(2):
            nodes_info['Factory'+str(i+1)] = {
                'initial_stock':initial_stocks[2+i], 'initial_shipments':initial_shipments[i],
                ...
                'destinations':['WholeSaler1','WholeSaler2'], 'dest_costs':[[dest_cost]*2]*num_products, 
                'ship_capacity':[ship_capacity]*2}
        for i in range(2):
            nodes_info['WholeSaler'+str(i+1)] = {
                'initial_stock':initial_stocks[4+i], 'initial_shipments':initial_shipments[2+i], 
                'stock_capacity':stock_capacities[4+i], 'stock_cost':stock_costs[4+i],
                'destinations':['Retailer1','Retailer2'], 'dest_costs':[[dest_cost]*2]*num_products, 
                'ship_capacity':[ship_capacity]*2}
        for i in range(2):
            nodes_info['Retailer'+str(i+1)] =  {
                'initial_stock':initial_stocks[6+i], 'initial_shipments':initial_shipments[4+i],
...
class SupplyChain2perStageSeasonalEnv(SupplyChain2perStageEnv):
    ...
    def __init__(self, num_products=1, initial_stocks=[800]*8, initial_supply=[[[600,600]],[[840,840]]],
                         initial_shipments=[[[600,600]],[[840,840]]]+[[[240,240]]]*4,
